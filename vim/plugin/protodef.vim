...
function! protodef#MakeMapping()
    if !exists('g:disable_protodef_mapping')
        nmap <buffer> <silent> <leader>z :set paste<cr>i<c-r>=protodef#ReturnSkeletonsFromPrototypesForCurrentBuffer({})<cr><esc>='[:set nopaste<cr>
        nmap <buffer> <silent> <leader>x :set paste<cr>i<c-r>=protodef#ReturnSkeletonsFromPrototypesForCurrentBuffer({'includeNS' : 0})<cr><esc>='[:set nopaste<cr>
    endif
endfunction
